"use strict";
var debug = require('debug')('mambo:search'),
    util = require('util'),
    Queue = require('./queue');

function Searchable(model){
    model.on('update', function(update){
        debug('New update on alias `'+update.alias +
            '` with hash `'+update.hash+'` ' + (update.range ? 'and range `' +
                update.range + '`' : ''));
        debug('Attribute updates: ' + util.inspect(update.updates));
        debug('Options: ' + util.inspect(update.options));
    });
