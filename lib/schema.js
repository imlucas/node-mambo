...
Schema.prototype.getAttributeDefinitions = function(){
    var defs = [];
    defs.push({
...
Schema.prototype.exportKey = function(hash, range){
    ...
    key[this.hash] = {};
    key[this.hash][this.hashType] = this.field(this.hash).export(hash);

    if(this.range){
        key[this.range] = {};
        key[this.range][this.rangeType] = this.field(this.range).export(range);
    }
    return key;
};
