...
var UpdateQuery = require('./update-query'),
    debug = require('debug')('mambo:query');
...
Query.prototype.fetch = function(done){
    ...
    if(this.filter){
        ...
        Object.keys(this.filter).forEach(function(key){
            var f = new Filter(schema, key, self.filter[key]);
            opts.conditions[key] = f.export();
        });
    }
...
function Filter(schema, key, data){
    ...
    if(data === new Object(data)){
        ...
        // @todo (lucas) dont think this datastructure will actually work with
