...
Batch.prototype.commit = function(){
    var d = Q.defer(),
        self = this;
    log.silly('Committing batch');
    if(this.requiredScans.length > 0){
        Q.all(this.requiredScans.map(function(scan){
            var schema = self.model.schema(scan.alias);
            return self.model.objects(scan.alias, scan.hash)
                .fields([schema.range])
                .fetch()
                .then(function(rows){
...
Batch.prototype.processCommit = function(d){
    log.silly('Process commit');
    if(this.numOps <= this.chunkSize){
        this.model.batchWrite(this.puts, this.deletes).then(d.resolve, d.reject);
    }
    else {
        Q.all(this.splitOpsIntoChunks().map(function(chunk){
            var p = Q.defer();
            this.model.batchWrite(chunk.puts, chunk.deletes).then(p.resolve, p.reject);
            return p.promise;
